...
- (void)setDataWithIndex:(id)data Index:(NSInteger)index {
    ...
    [super setData:data];
    int offset = index * 3;
    int count = [[(BookDetailInfo *)data labels] count];
    int leftCount = count - offset;
    leftCount = leftCount > 3 ? 3 : leftCount;
    switch (leftCount) {
        case 3:
            [_lblTag2 setText:[[(BookDetailInfo *)data labels] objectAtIndex:offset+2]];
            [_lblTag2 setHidden:NO];
        case 2:
            [_lblTag1 setText:[[(BookDetailInfo *)data labels] objectAtIndex:offset+1]];
            [_lblTag1 setHidden:NO];
        case 1:
            [_lblTag0 setText:[[(BookDetailInfo *)data labels] objectAtIndex:offset+0]];
            [_lblTag0 setHidden:NO];
        default: break;
    }
}
